...
*	AMXMOD script.
*	(plugin_gore2.sma)
*	by mike_cao <mike@mikecao.com> and fizzarum <tntmr2gg2@icqmail.com>
...
*	d - Gib explosion ( HE or damage higher than 250)
*
*/
...
public event_death()
{
	...
	if (iFlags&GORE_GIB && (equal(sWeapon,"grenade") || ((gHealthIndex[iVictim] - get_user_health(iVictim)) > 250)))
	{
		new iWeapon
		new iHitPlace
		new iAgressor = get_user_attacker(iVictim,iWeapon,iHitPlace)
...
{
		new rDistance = get_distance(origin,origin2)
    if(rDistance != 0){
...
public plugin_init()
{
	register_plugin("Plugin Gore2","1.03","mike_cao/fizzarum")
